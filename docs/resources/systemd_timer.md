...
A `.timer` unit. Pair it with the matching `.service` unit.
...
The catalog covers 16 directives from systemd v257 ([Unit], [Install], [Timer] and the
...
|-----------|----------|-------------|
| `name` | Required | Unit filename ending with `.timer`. Forces replacement when changed. |
