...
An `.automount` unit. Usually paired with the matching `.mount` unit.
...
```hcl
resource "systemd_automount" "demo" {
  name = "data.automount"
...
The catalog covers 4 directives from systemd v257 ([Unit], [Install], [Automount] and the
...
|-----------|----------|-------------|
| `name` | Required | Unit filename ending with `.automount`. Forces replacement when changed. |
