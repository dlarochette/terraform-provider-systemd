...
A `.slice` unit for the cgroup hierarchy.
...
The catalog covers 70 directives from systemd v257 ([Unit], [Install], [Slice] and the
...
|-----------|----------|-------------|
| `name` | Required | Unit filename ending with `.slice`. Forces replacement when changed. |
