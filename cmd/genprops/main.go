...
// LatestVersion is the newest systemd release covered by the catalog.
...
// documentation order.
var Catalogs = []VersionCatalog{
`

var byVersion = map[int]map[string][]directive{}

func sectionsOf(bySection map[string][]directive) []string {
	var order []string
...
func main() {
	...
	versions := []int{}
	byVersion := map[int]map[string][]directive{}
	for _, in := range args {
		...
		var v int
		if _, err := fmt.Sscanf(base, "load-fragment-v%d.gperf", &v); err != nil {
		...
		for sec, list := range bySection {
			...
			for _, d := range list {
				if prev, ok := used[d.Attr]; ok {
					fmt.Fprintf(os.Stderr, "attr collision in %s (v%d): %s vs %s\n", sec, v, prev, d.Name)
					os.Exit(1)
				}
				used[d.Attr] = d.Name
			}
		...
		byVersion[v] = bySection
	}
	slices.Sort(versions)

	fmt.Fprintf(&b, goHeader, versions[len(versions)-1])
	for _, v := range versions {
		fmt.Fprintf(&b, "\t{\n\t\tVersion: %d,\n\t\tSections: []SectionGroup{\n", v)
		for _, sec := range sectionsOf(byVersion[v]) {
			// keep the file order of the gperf
			fmt.Fprintf(&b, "\t\t\t{\n\t\t\t\tSection: %q,\n\t\t\t\tDirectives: []Directive{\n", sec)
			for _, d := range byVersion[v][sec] {
	...
	total := 0
	for _, bySection := range byVersion {
		for _, list := range bySection {
			total += len(list)
		}
	}
	fmt.Printf("wrote %s: %d versions (%v), %d directives total\n", *out, len(versions), versions, total)
}
